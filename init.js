...
(function () {
  ...
  function addSequence(seq, val, closure) {
    ...
    for (i = 0; i <= len; i++) {
      ...
      if (!chld) {
        prnt[prop] = chld =
          i === len ? { sequance: seq, value: val, closure: closure } : {};
      }
      prnt = chld;
    }
  }
  ...
  function appendContent(data) {
    ...
    if (
      !scrolling &&
      content.height() > $(window.document).height() * mobile_magic
    ) {
      scrolling = true;
  ...
  function addPromptLine() {
    ...
    appendContent(
      '<span id="' +
        id +
        '">&nbsp;<span id="lnCnt"></span><span id="cursor" class="inverse">&nbsp;</span><span id="lnSuf"></span></span>'
    );
    uiLineWrp = $("#" + id);
  ...
  function addNewLine(hidden) {
    ...
    appendContent(
      '<div id="' +
        ...
        '></span><span id="cursor" class="inverse">&nbsp;</span><span id="lnSuf"></span></div>'
    );
    uiLineWrp = $("#" + id);
    uiLineCnt = uiLineWrp.find("#lnCnt");
    uiLineSuf = uiLineWrp.find("#lnSuf");
